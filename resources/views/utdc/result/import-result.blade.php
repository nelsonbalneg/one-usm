...
@section('title')
     USM-AES| UTDC - Import Result From Excel
@endsection
    ...
    <div class="card">
        <div class="flex gap-3 p-4 text-sm rounded-md text-custom-500 bg-custom-50 dark:bg-custom-400/20">
            ...
            <div>
                ...
                <p><i data-lucide="check" class="inline-block size-4 mt-0.5 shrink-0"></i>The red color highlight indicates the presence of an existing CEE application number and/or duplicate data within the database.</p>
                <p><i data-lucide="check" class="inline-block size-4 mt-0.5 shrink-0"></i> The red-highlighted entries will be ignored by the system and will not be saved in the database.</p>
            </div>
        </div>
    </div>
            ...
            <div class="card" id="usersTable">
                ...
                <div class="border-dashed card-body border-y border-slate-200 dark:border-zink-500">
                    @if ($paginatedRows->isNotEmpty())
                        <form id="saveDataForm" action="{{ route('utdc.import.save') }}" method="POST">
                            @csrf
        ...
        <script>
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = "{{ route('utdc.cee-result.index') }}";
                }
            });
        </script>
    @endif
