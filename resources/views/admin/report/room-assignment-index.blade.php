...
@section('title')
    USM-AES | CEE - All Room Assigments
@endsection
    ...
    <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
        <div class="grow">
            ...
            <span
                ...
                ACTIVE CEE TERM: {{ $activeSession->name }}</span>
        </div>
                ...
                <div class="card-body">
                    <div class="flex items-center">
                        ...
                        <div class="flex items-center gap-2 space-x-4">
                            <a type="button" href="{{ route('admin.reservation.export.cee-examinees') }}"
                            ...
                            <a type="button" href="{{ route('admin.reservation.all-cee-examinee-report.pdf') }}"
                                class="bg-white border-dashed shrink-0 text-custom-500 btn border-custom-500 hover:text-custom-500 hover:bg-custom-50 hover:border-custom-600 focus:text-custom-600 focus:bg-custom-50 focus:border-custom-600 active:text-custom-600 active:bg-custom-50 active:border-custom-600 dark:bg-zink-700 dark:ring-custom-400/20 dark:hover:bg-custom-800/20 dark:focus:bg-custom-800/20 dark:active:bg-custom-800/20"><i
                                    class="align-baseline ltr:pr-1 rtl:pl-1 ri-download-2-line"></i> Export to PDF</a>
                            <!-- Dropdown for Filter -->
    ...
    <script>
        ...
        function loadData() {
            ...
            var table = $('#dbData').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.reservation.room.view-all-applicant') }}",
                    type: "GET",
                    dataType: "JSON",
                    data: function(d) {
                        ...
                        render: function(data, type, row) {
                            if (data) {
                                ...
                                return date.toLocaleDateString('en-US',
                                    options); // Format like "May 5, 2004"
                            }
                            return '';
                        }
                    },
                ...
                drawCallback: function(settings) {
                    // Implement row grouping by Batch and then by Room
                    let api = this.api();
                    let rows = api.rows({
                        page: 'current'
                    }).nodes();
                    ...
                    api.column(2, {
                        page: 'current'
                    }).data().each(function(batch, i) {
                        if (lastBatch !== batch) {
                            $(rows).eq(i).before(
                                `<tr class="group-batch" style="background-color: #e0f7fa; color: #00796b;"><td colspan="7">Batch No.: <b> ${batch}</b></td></tr>`
                            );
                            lastBatch = batch;
                            lastRoom = null; // Reset the lastRoom for a new batch
                        }

                        let room = api.column(4, {
                            page: 'current'
                        }).data()[i];
                        if (lastRoom !== room) {
                            $(rows).eq(i).before(
                                `<tr class="group-room" style="background-color: #f0f8ff; color: #333333;"><td colspan="7" style="padding-left: 20px;">Room: ${room}</td></tr>`
                            );
                            lastRoom = room;
                        }
                    });
