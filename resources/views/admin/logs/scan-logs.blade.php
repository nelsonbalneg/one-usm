...
@section('contents')
    <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
        <div class="grow">
            <h5 class="uppercase text-16">USM-Admission and Enrollment System | QR Logs</h5>
            <span
                ...
                ACTIVE CEE TERM: {{ $cee_term_active->name }}</span>
        </div>
        <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
            <li
                class="relative before:content-['\ea54'] before:font-remix ltr:before:-right-1 rtl:before:-left-1  before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:text-slate-400 dark:text-zink-200">
                <a href="#!" class="text-slate-400 dark:text-zink-200">Scan Logs</a>
            </li>
        </ul>
    </div>
    ...
    <script>
        ...
        function loadData() {
            ...
            var table = $('#dbData').DataTable({
                ...
                deferRender: true,
                ajax: {
                    url: "{{ route('admin.cee.scan-logs-fetch-all-data') }}",
                    type: "GET",
                    dataType: "JSON",
                    headers: {
