...
@section('contents')
    ...
    <div class="overflow-x-auto space-y-4"> <!-- Adds vertical spacing between child elements -->
        ...
        @if (!empty($fetchSectionSchedules))
            <table class="w-full">
                <thead class="ltr:text-left rtl:text-right">
                    <tr>
                        <th class="w-auto px-3.5 py-2.5 font-semibold border-b border-slate-200">SUBJECT</th>
                        <th class="w-[250px] px-3.5 py-2.5 font-semibold border-b border-slate-200"></th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($fetchSectionSchedules as $fetchSectionSchedule)
                        <tr>
                            <td class="px-3.5 py-2.5 border-y border-slate-200">
                                <div class="flex items-center gap-2 mb-1">
                                    <div class="grow">
                                        <span class="delivery_status px-2.5 py-0.5 text-xs inline-block font-medium rounded border bg-sky-100 border-sky-200 text-sky-500">
                    ...
                    @empty
                        <tr>
                            <td colspan="2" class="text-center px-3.5 py-2.5 border-y border-slate-200">
                                <h4><i>No data available</i></h4>
                            </td>
                        </tr>
                    @endforelse
        ...
        <!-- Add spacing after all content -->
        <div class="h-24"></div> <!-- Replace <br> with height spacing (6x4 = ~96px) -->
    </div>
</div>
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            ...
                            <div class="xl:col-span-4">
                                ...
                                <select class="form-input border-slate-300 focus:outline-none focus:border-custom-500"
                                    ...
                                    <option value="">-- Select Grant Template --</option>
...
@push('scripts')
    ...
    <script>
        const baseScheduleRoute =
            `{{ route('aro.schedules.get', ['subjectId' => '__SUBJECT__', 'termId' => '__TERM__']) }}`;
        ...
        function selectSection(sectionId) {
            ...
            const apiUrl = `{{ route('aro.section.select', ['sectionId' => '__SECTION__']) }}`
        ...
        function setTransactionType(selectElement) {
            ...
            const apiUrl = `{{ route('aro.transaction.type.update', ['id' => '__ID__']) }}`.replace('__ID__',
                trialProgramId);
        ...
        function setClassSection(classSectionId, classSectionName) {
            ...
            fetch("{{ route('aro.class.section.update', ['id' => ':id']) }}".replace(':id', trialProgramId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
        ...
        function setYearLevel(yearLevelId, yearLevelName) {
            ...
            fetch("{{ route('aro.year.level.update', ['id' => ':id']) }}".replace(':id', trialProgramId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': "{{ csrf_token() }}",
        ...
        function setCurriculum(curriculumId, curriculumName) {
            ...
            fetch("{{ route('aro.curriculum.update', ['id' => ':id']) }}".replace(':id', trialProgramId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': "{{ csrf_token() }}",
        ...
        function setTableFee(tableFeeId, tableFeeName) {
            ...
            fetch("{{ route('aro.table.fee.update', ['id' => ':id']) }}".replace(':id', trialProgramId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': "{{ csrf_token() }}",
        ...
        function setScholarship(scholarshipId, scholarshipName) {
            ...
            fetch("{{ route('aro.scholarship.provider.update', ['id' => ':id']) }}".replace(':id', trialProgramId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': "{{ csrf_token() }}",
        ...
        function setgrantTemplate(grantTemplateId, grantTemplateName) {
            ...
            fetch("{{ route('aro.grant.template.update', ['id' => ':id']) }}".replace(':id', trialProgramId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': "{{ csrf_token() }}",
        ...
        function removeSubjects(subjectId, subjectName) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch("{{ route('aro.subjects.remove') }}", {
                            method: 'DELETE',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': "{{ csrf_token() }}"
        ...
        function selectSchedule(subjectDetailId, trialProgramId, scheduleId, campusId) {
            ...
            fetch("{{ route('aro.class.schedules.update', ['id' => ':id']) }}".replace(':id', subjectDetailId), {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': "{{ csrf_token() }}"
        ...
        function processEnrollment(trialProgramId, tenantId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch("{{ route('aro.enrollment.process', ['id' => '__ID__']) }}".replace('__ID__',
                            trialProgramId), {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                        ...
                        .then(data => {
                            if (data.status === 'success') {
                                ...
                                const route = "{{ route('aro.sar.students.list', ['id' => '__ID__']) }}"
