...
@push('scripts')
   ...
   <script>
    var homeLocation = [7.2464105, 124.5118993]; // center from Google Maps
    ...
    var map = L.map('viewer', {
        zoomControl: false,
        minZoom: 17, // zoom in close
        maxZoom: 22,
        // Max bounds around campus (slightly more room to the right)
        maxBounds: [
            [7.2464105 - 0.0018, 124.5118993 - 0.0005], // southwest corner
            [7.2464105 + 0.0018, 124.5118993 + 0.0035]  // northeast corner
        ]
    ...
    // Add campus buildings from GeoJSON
    fetch('/geojson/campus-buildings.geojson') // adjust path as needed
        .then(res => res.json())
        .then(data => {
            L.geoJSON(data, {
                style: {
                    color: '#555',       // outline color
                    fillColor: '#888',   // building fill
                    weight: 1,
                    opacity: 0.7,
                    fillOpacity: 0.5
                }
