...
class CustomPDF extends FPDF
{
    function Footer()
    {
        ...
        $this->Cell(0, 4, 'https://cee.usm.edu.ph', 0, 1, 'L');

        $this->SetFont('Roboto', '', 8); // italic
        $this->SetTextColor(150, 150, 150); // lighter gray
        $this->SetXY(15, $this->GetY());
        $this->Cell(0, 4, 'University Information and Communication Technology (ICTC)', 0, 1, 'L');
    }
}
...
class ReportController extends Controller
{
    public function index()
    {
        ...
        return view("admin.report.report", compact('activecee_session', 'ceeSessions'));
    }
    ...
    public function getroomData(Request $request)
    {
        if ($request->ajax()) {
            $data = DB::table('reservations')
                ...
                ->orderBy('reservation_count', 'asc')  // Sort by lastname in ascending order
                ->get();

            return DataTables::of($data)
                ->addColumn('action', function ($row) {
                    return '<div class="flex gap-3">
                            <a href=' . route('admin.cee.room-adjustment.index', ['roomId' => $row->id]) . ' target="_blank" class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md view-detail size-8 bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500"><i data-lucide="eye" class="inline-block size-3"></i> </a>
                            <a href=' . route('admin.reservation.room.view-applicacant-by-room', ['roomId' => $row->id]) . ' target="_blank" class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500"><i data-lucide="download" class="size-4"></i></a>
                        </div>';
                })
    ...
    public function viewApplicantsByRoom(Request $request)
    {
        ...
        $pdf = PDF::loadView('admin.report.room-assignment', compact('data', 'roomDetails'));
    ...
    public function allroomIndex(Request $request)
    {
        ...
        return view('admin.report.room-assignment-index', compact('ceeSessions', 'activeSession'));
    }
    ...
    // private function addCustomFooter(FPDF $pdf)
    // {
    //     $pdf->SetY(45);
    //     $pdf->SetFont('Roboto', '', 8);
    //     $pdf->SetTextColor(128, 128, 128);
    //     $pdf->Cell(0, 10, 'University of Southern Mindanao - Confidential', 0, 0, 'C');
    //     $pdf->SetFont('Roboto', '', 8);
    ...
    public function generateAllCeeExamineePdf(Request $request)
    {
        ...
        $response = new StreamedResponse(function () {
            ...
            $pdf = new CustomPDF('P', 'mm', [215.9, 330.2]); // Folio size (8.5 x 13 inches)
    ...
    public function applicantViewIndexByCeeSession()
    {
        ...
        return view('admin.report.view-applicant-by-exam-session', compact('ceeActiveSession', 'ceeSessions'));
    }
        ...
        if ($request->ajax()) {
            $data = DB::table('users as u')
                ->select([
                    ...
                    END AS campus_name"),
                    DB::raw("CASE
                        WHEN r.campus_id_prio_prog_2 = 1 THEN 'USM-Main'
                        WHEN r.campus_id_prio_prog_2 = 3 THEN 'USM KCC'
                        WHEN r.campus_id_prio_prog_2 = 5 THEN 'USM PALMA CLUSTER'
                        WHEN r.campus_id_prio_prog_2 = 6 THEN 'USM MLANG'
                        WHEN r.campus_id_prio_prog_2 = 7 THEN 'USM Antipas'
                        WHEN r.campus_id_prio_prog_2 = 8 THEN 'USM Pigcawayan'
                        ELSE 'Unknown'
                    END AS campus_name_2"),
                    DB::raw("CASE
                        WHEN r.campus_id_prio_prog_3 = 1 THEN 'USM-Main'
                        WHEN r.campus_id_prio_prog_3 = 3 THEN 'USM KCC'
                        WHEN r.campus_id_prio_prog_3 = 5 THEN 'USM PALMA CLUSTER'
                        WHEN r.campus_id_prio_prog_3 = 6 THEN 'USM MLANG'
                        WHEN r.campus_id_prio_prog_3 = 7 THEN 'USM Antipas'
                        WHEN r.campus_id_prio_prog_3 = 8 THEN 'USM Pigcawayan'
                        ELSE 'Unknown'
                    END AS campus_name_3"),
                    'r.app_no',
                    'r.firstpriorty_desc',
            ...
            return Datatables::of($data)
                ...
                ->editColumn('app_no', function ($row) {
                    ...
                    return '
                        <div class="flex flex-col items-start">
                          <span class="inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded-md bg-green-100 text-green-600">
                                ' . $row->name . '
                            </span>
                            <span class="font-semibold leading-tight uppercase text-slate-700">' . $fullname . '</span>

                            <span class="inline-block mt-1 px-2 py-0.5 text-xs font-medium rounded-md bg-purple-100 text-purple-600">
                                ' . $appNo . '
                            </span>
                        </div>
                    ';
                ...
                ->editColumn('contact', function ($row) {
                    ...
                    <div class="text-sm leading-relaxed">
                        ...
                        <div class="flex items-center gap-2 mt-1 text-gray-700">
                ...
                ->editColumn('first_priority_program', function ($row) {
                    return '
                    <div class="text-sm leading-relaxed">
                        <div class="flex items-center gap-2 text-gray-700">
                            <i data-lucide="graduation-cap" class="w-4 h-4 "></i>
                            ' . e($row->firstpriorty_desc) . '
                        </div>
                        <div class="flex items-center gap-2 mt-1 text-gray-700">
                ...
                ->editColumn('secondpriority_desc', function ($row) {
                    return '
                    <div class="text-sm leading-relaxed">
                        <div class="flex items-center gap-2 text-gray-700">
                            <i data-lucide="graduation-cap" class="w-4 h-4 "></i>
                            ' . e($row->secondpriority_desc) . '
                        </div>
                        <div class="flex items-center gap-2 mt-1 text-gray-700">
                            <i data-lucide="building" class="w-4 h-4 "></i>
                            ' . e($row->campus_name_2) . '
                ...
                ->editColumn('thirdpriorty_desc', function ($row) {
                    return '
                    <div class="text-sm leading-relaxed">
                        <div class="flex items-center gap-2 text-gray-700">
                            <i data-lucide="graduation-cap" class="w-4 h-4 "></i>
                            ' . e($row->thirdpriorty_desc) . '
                        </div>
                        <div class="flex items-center gap-2 mt-1 text-gray-700">
                            <i data-lucide="building" class="w-4 h-4 "></i>
                            ' . e($row->campus_name_3) . '
                ...
                ->rawColumns(['app_no', 'contact', 'first_priority_program', 'secondpriority_desc', 'thirdpriorty_desc'])
                ->make(true);
        }
    }
