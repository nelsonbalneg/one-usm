...
class RoomAdjustmentController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('admin.rooms.room-adjustment', compact('room'));
    }
    ...
    public function getRoomsByExamSession(Request $request)
    {
        ...
        if (!$activeSession) {
            return response()->json(['message' => 'No active CEE session found'], 404);
        }

        // Retrieve rooms only for the active session
        $rooms = Room::where('cee_session_id', $activeSession->id)
            ->where('capacity', '>', 0)
            ->get(['id', 'room_name', 'capacity', 'college_name', 'status']);
    ...
    public function update(Request $request, string $id)
    {
        ...
        //   Mail::to($data->email)->send(new RoomStatusNotification((array) $data)); // Cast $data to an array
