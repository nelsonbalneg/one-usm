                ...
                <div class="card-body">
                    <div class="flex items-center gap-3">
                        <div class="rounded-full w-28 h-28 bg-slate-100 shrink-0 dark:bg-zink-600">
                            <img src="http://172.16.0.43/uploads/{{ basename($photo) }}" alt=""
                                class="object-cover w-full h-full rounded-full">
                        </div>
                        <div class="grow">
                            <h3 class="text-[20px]">{{ $fullname }}
                        ...
                        <div class="mt-5 tab-content">
                            <div class="block tab-pane" id="curriculum">
                                ...
                                <div class="overflow-x-auto">
                                    @if (!empty($curriculum['yearAndLevel']))
                                        <table class="w-full">
                                            ...
                                            <tbody>
                                                @foreach ($curriculum['yearAndLevel'] ?? [] as $row)
                                ...
                                <div class="overflow-x-auto">
                                    <select class="form-input border-slate-300 focus:outline-none focus:border-custom-500"
                                        ...
                                        @foreach ($sectionList as $section)
                                            <option value="{{ $section['sectionId'] }}">{{ $section['sectionName'] }}
                                            </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="flex justify-center gap-2 mt-4">
                            @if ($countSubjects > 0)
                                <button type="button"
                                    ...
                                    {{ $status_id == 1 ? 'Process Re-Assessment' : 'Process Enrollment' }}
                                </button>
                            @endif
                        </div>
                    </div>
        ...
        function processEnrollment(trialProgramId, tenantId) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch("{{ route('pao.enrollment.process.freshmen', ['id' => '__ID__']) }}".replace('__ID__',
                            trialProgramId), {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                        ...
                        .then(data => {
                            if (data.status === 'success') {
                                ...
                                window.location.href = "{{ route('pao.students.index') }}";
                            } else {
