...
<body>
    <div class="container">
        <div class="header">
            ...
            <br>
            <div class="title"><strong>({{ $roomDetails->campus }}) {{ $roomDetails->college_name }} -
                    {{ $roomDetails->room_name }}</strong></div>
            <div class="title"><strong> {{ \Carbon\Carbon::parse($roomDetails->schedule)->format('F j, Y') }} |
                    {{ $roomDetails->time }}</strong></div>
        </div>
    ...
    <div class="footer">
        <p>Downloaded Date and Time: {{ \Carbon\Carbon::now()->setTimezone('Asia/Manila')->format('Y-m-d h:i:s A') }}
        </p>
        <p style="margin-top: -8px;"> <i>University of Southern Mindanao - College Entrance Examination Reservation
                System v4.0 | <b> Powered by: UICTO</b></i></p>
    </div>
