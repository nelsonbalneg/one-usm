    ...
    <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
        <div class="grow">
            ...
            <span
                ...
                ACTIVE CEE TERM: {{ $ceeActiveSession->name }}</span>
        </div>
            ...
            <div class="card" id="usersTable">
                <div class="card-body">
                    <div class="flex items-center">
                        <h6 class="uppercase text-15 grow">List of all Applicants</h6>
                        <div class="flex items-center gap-2 space-x-4">
                            <a type="button" href="{{ route('admin.reservation.export.applicantByActiveCeeSession') }}"
                                        ...
                                        <select id="cee-term-select" name="cee_session_id" data-choices
                                            ...
                                            @foreach ($ceeSessions as $session)
                                                <option value="{{ $session->id }}"
                                                    {{ isset($ceeActiveSession) && $ceeActiveSession->id == $session->id ? 'selected' : '' }}>
                                                    {{ $session->name }}
                                                </option>
                                            @endforeach
                                        </select>
    ...
    <script>
        ...
        function loadData() {
            ...
            var table = $('#dbData').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.reservation.view-applicant-by-active-cee-sessison.view') }}",
                    type: "GET",
                    dataType: "JSON",
                    data: function(d) {
                ...
                columns: [{
                    ...
                    {
                        data: "first_priority_program",
                        name: "first_priority_program"
                    },
                    {
