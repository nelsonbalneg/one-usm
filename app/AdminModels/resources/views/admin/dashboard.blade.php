...
@section('contents')
    <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
        <div class="grow">
            <h5 class="text-16">USM Admission and Enrollment System</h5>
        </div>
            ...
            <div class="relative card-body">
                <div class="grid items-center grid-cols-12">
                    <div class="col-span-12 lg:col-span-8 2xl:col-span-7">
                        ...
                        <p class="mb-5 text-slate-400">University of Southern Mindanao - Admissions and Enrollment System
                        </p>
                        <a type="button" href="{{ route('admin.exam-session.index') }}" target="_blank"
                            class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-500/20 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-500/20 dark:ring-custom-400/20">CEE
                            Exam Term: {{ $ceesessionactive->name }} - [{{ $ceesessionactive->id }}]</a>
                        <a type="button" href="{{ route('admin.cee.online-users.index') }}" target="_blank"
                            class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-500/20 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-500/20 dark:ring-custom-400/20">Online:
                            {{ $onlineUserCount }}</a>
                    </div>
        ...
        <div class="col-span-12 card md:col-span-6 lg:col-span-3 2xl:col-span-2">
            <div class="text-center card-body">
                ...
                <h5 class="mt-4 mb-2"><span class="counter-value"
                        data-target="{{ $userCount }}">{{ $userCount }}</span></h5>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var rawData = {!! json_encode($sexStatistics) !!};

            //Add total beside session name (e.g., "SY 2025-2026 (Total: 25636)")
            var categories = Object.keys(rawData).map(session => {
                const total = rawData[session].total;
                return `${session} (Total: ${total})`;
            });

            var dataSeries = Object.keys(rawData).map(session => {
                const male = rawData[session].male;
                const female = rawData[session].female;

                return {
                    x: `${session} (${male + female})`,
                    y: [Math.min(male, female), Math.max(male, female)],
                    goals: [{
                            name: "Male",
                            value: male,
                            strokeColor: "#0ea5e9"
            ...
            var options = {
                ...
                dataLabels: {
                    ...
                    formatter: function(val, opts) {
                        const {
                            goals
                        } = opts.w.globals.initialSeries[opts.seriesIndex].data[opts.dataPointIndex];
                        return `M:${goals[0].value} | F:${goals[1].value}`;
                    }
                },
                xaxis: {
                    title: {
                ...
                tooltip: {
                    ...
                    <div style="padding:5px">
                        ...
                        👨 Male: ${goals[0].value}<br>
                        👩 Female: ${goals[1].value}<br>
                    </div>
                `;
                    }
                }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('{{ route('admin.reservations.per-day') }}')
    ...
    {{-- Reservation per mun, school and priority --}}
    <script>
        ...
        function loadData() {
            ...
            var loadData = $('#dbData').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.reservation.data-first-priority') }}",
                    type: "GET",
                    dataType: "JSON"
                },
                columns: [{
        ...
        function loadDataMun() {
            ...
            var loadData = $('#dataMun').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.reservation.data-by-municipility') }}",
                    type: "GET",
                    dataType: "JSON"
                },
                columns: [{
        ...
        function loadDataSchool() {
            ...
            var loadData = $('#dataSchool').DataTable({
                ...
                deferRender: true, // Only render rows when visible
                ajax: {
                    url: "{{ route('admin.reservation.data-by-school') }}",
                    type: "GET",
                    dataType: "JSON"
                },
                columns: [{
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('{{ route('admin.reservation.percentage') }}')
                .then(response => response.json())
                .then(data => {
    ...
    {{-- confirmed reservation turnout --}}
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const chartElement = document.querySelector('#gradientChart2');

            // Fetch percentage from the backend
            fetch('{{ route('admin.confirmed-reservation.percentage') }}')
                .then(response => response.json())
                .then(data => {
                    const percentage = data.percentage;
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            fetch("{{ route('admin.reservation-stackbar') }}")
                .then(response => response.json())
                .then(data => {
                    var options = {
                        series: data.series,
