    ...
    <div class="layout-width">
        <div
            ...
            <div
                ...
                {{-- <!-- Search Bar -->
                <div class="relative hidden lg:block group-data-[layout=horizontal]:hidden">
                    <input type="text" id="search-input"
                        class="py-2 pr-4 text-sm text-topbar-item bg-topbar border border-topbar-border rounded pl-8 placeholder:text-slate-400 form-control focus-visible:outline-0 min-w-[300px] focus:border-blue-400 ..."
                        placeholder="Search for ..." autocomplete="off">
                    <i data-lucide="search" class="inline-block size-4 absolute left-2.5 top-2.5 ..."></i>

                    <!-- Dropdown for results -->
                    <div id="search-results"
                        class="absolute z-10 hidden w-full mt-1 overflow-y-auto bg-white border border-gray-300 rounded shadow-lg max-h-64">
                        <!-- Loading indicator -->
                        <div id="loading-indicator" class="hidden px-4 py-2 text-gray-500">Loading...</div>
                    </div>
                </div> --}}
                ...
                <div class="flex gap-3 ms-auto">
                    ...
                    <div class="relative flex items-center dropdown h-header">
                        ...
                        <div class="absolute z-50 hidden p-4 ltr:text-left rtl:text-right bg-white rounded-md shadow-md !top-4 dropdown-menu min-w-[14rem] dark:bg-zink-600"
                            ...
                            <h6 class="mb-2 text-sm font-normal text-slate-500 dark:text-zink-300">USMCEE Admin</h6>
                            ...
                            <ul>
                                <li>
                                    <a class="block ltr:pr-4 rtl:pl-4 py-1.5 text-base font-medium transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:text-custom-500 focus:text-custom-500 dark:text-zink-200 dark:hover:text-custom-500 dark:focus:text-custom-500"
                                        href="{{ route('admin.profile.edit') }}"><i data-lucide="user-2"
                                            class="inline-block size-4 ltr:mr-2 rtl:ml-2"></i> My Profile</a>
                                </li>
                ...
                debounceTimer = setTimeout(() => {
                    axios.get('/search', {
                        ...
                        .then(response => {
                            ...
                            if (results.reservations.length || results.users.length) {
                                // Add reservations
                                results.reservations.forEach(reservation => {
                                    ...
                                    item.innerHTML =
                                        `App No: <span class="font-bold">${reservation.app_no}</span>`;
                                ...
                                results.users.forEach(user => {
                                    ...
                                    item.className =
                                        'px-4 py-2 hover:bg-gray-100 cursor-pointer';
                                    item.innerHTML =
                                        `Name: <span class="font-bold">${user.name}</span> - Email: <span class="font-bold">${user.email}</span>`;
                                    item.addEventListener('click', () => {
                        ...
                        .catch(error => {
                            ...
                            resultsContainer.innerHTML =
                                '<div class="px-4 py-2 text-red-500">Error loading results</div>';
                        });
                }, 300); // 300ms debounce
            } else {
