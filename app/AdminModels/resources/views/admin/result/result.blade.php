    ...
    <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
        <div class="grow">
            ...
            <span
                ...
                ACTIVE CEE EXAM TERM: {{ $ceeSession->name }}</span>
        </div>
                ...
                <div class="card" id="usersTable">
                    <div class="card-body">
                        ...
                        <div class="grid grid-cols-1 gap-2 xl:grid-cols-12">
                            ...
                            <div class="xl:col-span-4">
                                <form action="{{ route('admin.import.preview') }}" method="POST"
                                    ...
                                    <div class="flex items-center gap-2">
                                        <input type="file" id="file-upload" name="file" name="file"
                                            accept=".xls,.xlsx"
                            ...
                            <div class="xl:col-span-4 mt-7">
                                <a href="{{ route('admin.reservation.export.confirmed-status') }}" id="upload-result-temp"
                    ...
                    <div class="border-dashed card-body border-y border-slate-200 dark:border-zink-500">
                        <div class="flex items-center mb-4">
                            {{-- for {{ $ceeSession->name }}[{{ $ceeSession->id }}] --}}
                            <h6 class="text-15 grow" style="text-transform: uppercase;">CEE RESULT Records</h6>
        ...
        <script>
            ...
            function loadData() {
                ...
                var table = $('#dbData').DataTable({
                    ...
                    ajax: {
                        url: "{{ route('admin.cee.result.fetch-data') }}",
                        type: "GET",
                        dataType: "JSON",
                        data: function(d) {
            ...
            $('#editForm').submit(function(event) {
                ...
                $.ajax({
                    url: '/admin/cee-result/' + resultId, // Replace with your endpoint URL
            ...
            $('body').on('click', '.unpost-entry', function(event) {
                event.preventDefault();
            ...
            $('body').on('click', '.post-entry', function(event) {
                event.preventDefault();

                let posturl = $(this).attr('href');
                let resultId = posturl.split('/').pop();

                Swal.fire({
                    title: 'Enter your password to post the result',
                    input: 'password',
                    inputPlaceholder: 'Type your password',
                    showCancelButton: true,
                    confirmButtonText: 'Submit',
                    ...
                    if (result.isConfirmed) {
                        ...
                        // AJAX call to verify password and update status
                        $.ajax({
                            url: posturl, // Backend route to verify and update
                            type: 'POST',
                            data: {
                                password: password,
                                result_id: resultId,
            ...
            $('#addForm').submit(function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.cee-result.store') }}",
                    method: 'POST', // Use POST method
                    data: formData,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
