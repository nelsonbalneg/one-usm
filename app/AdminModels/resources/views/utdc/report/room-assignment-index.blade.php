...
@section('title')
     USM-AES | UTDC - All Room Assigments
@endsection
                ...
                <div class="card-body">
                    <div class="flex items-center">
                        ...
                        <div class="flex items-center gap-2 space-x-4">
                            <a type="button" href="{{ route('utdc.reservation.export.cee-examinees') }}"
                            ...
                            <a type="button" href="{{ route('utdc.reservation.all-cee-examinee-report.pdf') }}"
                                class="bg-white border-dashed shrink-0 text-custom-500 btn border-custom-500 hover:text-custom-500 hover:bg-custom-50 hover:border-custom-600 focus:text-custom-600 focus:bg-custom-50 focus:border-custom-600 active:text-custom-600 active:bg-custom-50 active:border-custom-600 dark:bg-zink-700 dark:ring-custom-400/20 dark:hover:bg-custom-800/20 dark:focus:bg-custom-800/20 dark:active:bg-custom-800/20"><i
    ...
    <script>
        ...
        function loadData() {
            ...
            var table = $('#dbData').DataTable({
                ...
                ajax: {
                    url: "{{ route('utdc.reservation.room.view-all-applicant') }}",
                    type: "GET",
                    dataType: "JSON",
                    data: function(d) {
                ...
                drawCallback: function(settings) {
                    ...
                    api.column(8, {
                        ...
                        if (lastBatch !== batch) {
                            $(rows).eq(i).before(
                                `<tr class="group-batch" style="background-color: #e0f7fa; color: #00796b;"><td colspan="13">Batch No.: <b> ${batch}</b></td></tr>`
                            );
                            lastBatch = batch;
                            lastRoom = null; // Reset the lastRoom for a new batch
                        }

                        let room = api.column(10, {
                            page: 'current'
                        }).data()[i];
                        if (lastRoom !== room) {
                            $(rows).eq(i).before(
                                `<tr class="group-room" style="background-color: #f0f8ff; color: #333333;"><td colspan="13" style="padding-left: 20px;">Room: ${room}</td></tr>`
                            );
                            lastRoom = room;
                        }
                    });
