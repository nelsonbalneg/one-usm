    ...
    <div class="grid grid-cols-1 gap-x-5 xl:grid-cols-12">
        ...
        <div class="xl:col-span-12 md:col-span-12">
            <div class="card sticky top-[calc(theme('spacing.header')_*_1.3)]">
                <div class="card-body">
                    <h6 class="mb-0 text-lg font-semibold text-blue-500 uppercase">PROGRAMS TAGGED TO {{ $paoUsers[$selectedPrograms->first()['policyId']] ?? 'N/A' }} <br>
                    ...
                    <table class="w-full table-auto">
                        ...
                        <tbody>
                            @foreach ($selectedPrograms as $program)
                                <tr>
                                    <td class="px-4 py-2 border"><a class="block px-4 py-1.5 text-base transition-all duration-200 ease-linear text-slate-600 dropdown-item hover:bg-slate-100 hover:text-slate-500 focus:bg-slate-100 focus:text-slate-500 dark:text-zink-100 dark:hover:bg-zink-500 dark:hover:text-zink-200 dark:focus:bg-zink-500 dark:focus:text-zink-200" href="{{ route('aro.sar.students.list', ['id' => $program['policyId']]) }}">{{ $program['policyId'] }}</a></td>
